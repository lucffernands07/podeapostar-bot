...
def get_perc_ranking(team_id):
    ...
    try:
        ...
        count = sum(1 for f in fixtures if (f['goals']['home'] or 0) + (f['goals']['away'] or 0) > 1.5)
...
def executar_cashout():
                ...
                if fixtures:
                    for f in fixtures:
                        if f['fixture']['status']['short'] == "NS":
                            ...
                            if hora_br not in horarios_agrupados: horarios_agrupados[hora_br] = []
    ...
    for hora in sorted(horarios_agrupados.keys()):
        ...
        # REGRA MESTRA: Mínimo de 5 jogos no mesmo horário
        if len(jogos_brutos) >= 5:
            ...
            for f in jogos_brutos:
                ...
                # Regra: Prioridade 85% casa ou média
