"""
REGRAS DE MERCADO - FALTAS TOTAIS DO JOGO (FORMATO DECIMAL LIVRE)
Retorna a média esperada calculada dos últimos 5 jogos para livre escolha na Betano.
"""

def verificar_faltas_totais(s):
    if not isinstance(s, dict):
        return []

    # Lê as médias já calculadas pelo scraper dos últimos 5 jogos (CASA para o mandante / FORA para o visitante)
    media_m = float(s.get("mandante_media_faltas_casa", 0) or 0)
    media_v = float(s.get("visitante_media_faltas_fora", 0) or 0)
    ...
    # 2. Retorna a média exata e limpa, mantendo a chave padrão FALTAS_JOGO_TOTAL
    mercados_aprovados.append({
        "mercado": f"Faltas Totais no Jogo: {media_esperada:.1f}",
        "tipo": "FALTAS_JOGO_TOTAL"
    })
