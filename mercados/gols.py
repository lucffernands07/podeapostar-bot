...
"""
REGRAS DE GOLS - ATUALIZADO COM TRAVAS ANTI-GOLEADA
Proteção ajustada para Under 3.5 / 4.5 e Overs refinados.
"""
...
def verificar_gols(s):
    ...
    # ----------------------------------------------------------
    # 🚨 TRAVAS DE SEGURANÇA CONTRA GOLEADAS (CASO SHAKHTAR)
    # ----------------------------------------------------------
    # Visitante frágil (tomou 7+ gols fora) OU Mandante rolo compressor (fez 8+ gols em casa)
    ...
    # ----------------------------------------------------------
    if pct_15 > 0:
        if m_jogos_marcou_casa >= 3 and (v_jogos_marcou_fora >= 3 or v_sofridos_fora >= 7):
            if media_total_confronto >= 2.0 or mandante_avassalador:
    ...
    # ----------------------------------------------------------
    if pct_25 > 0:
        if v_sofridos_fora >= 8 or media_total_confronto >= 2.8 or mandante_avassalador:
            mercados_aprovados.append({"mercado": f"+2.5 Gols ({pct_25}%)", "tipo": "GOLS_25"})

    # ----------------------------------------------------------
    # 🔴 UNDERS -3.5 E -4.5
    # Só aprova se NÃO houver risco de goleada pelo visitante frágil ou mandante forte
    # ----------------------------------------------------------
    pode_apostar_under = not (visitante_peneira or mandante_avassalador)

    if pode_apostar_under:
        if pct_m35 > 0 and media_total_confronto <= 2.5:
            mercados_aprovados.append({"mercado": f"-3.5 Gols ({pct_m35}%)", "tipo": "GOLS_M35"})
            
        if pct_m45 > 0 and media_total_confronto <= 3.0:
