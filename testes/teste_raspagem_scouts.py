...
def extrair_scouts_por_aba(driver, url_base, mid_param, mercado, dicionario_escudos, acumulador_scouts):
    ...
    print(f"      🔍 [DEBUG] Acessando {mercado}: {url_final}")
    ...
    try:
        ...
        print(f"      🔍 [DEBUG] Coluna alvo identificada no índice: {indice_alvo}")
        ...
        for linha in linhas:
            try:
                ...
                if len(celulas) > indice_alvo:
                    ...
                    # LOG DE DIAGNÓSTICO
                    if valor_txt and valor_txt.isdigit() and int(valor_txt) > 0:
                        print(f"      ✅ [DEBUG] JOGADOR: {nome_jogador} | TIME: {time_real} | VALOR: {valor_txt}")
