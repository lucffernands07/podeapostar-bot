...
def buscar_gols_pelas_linhas_contadas(driver, id_jogo):
    ...
    print(f"🌐 Acessando Gols: {url}", flush=True)
    driver.get(url)
    time.sleep(15) 
    ...
    # Índices baseados na sua contagem (Linha - 1)
    ...
    for mercado, idx in mapeamento.items():
        try:
            if idx < len(linhas):
                ...
                print(f"📍 Linha {idx+1} ({mercado}): {dados}", flush=True)
        except:
            continue
    return res

def buscar_mercado_simples(driver, url, mercado):
    print(f"🌐 Acessando {mercado}: {url}", flush=True)
    driver.get(url)
    time.sleep(10)
    linhas = driver.find_elements(By.CSS_SELECTOR, ".ui-table__row")
...
def rodar_teste():
    ...
    try:
        print("🚀 [TESTE] Iniciando captura...", flush=True)
        
        gols = buscar_gols_pelas_linhas_contadas(driver, id_jogo)
        
        # Correção aqui: garantindo que btts receba 1 valor e dc receba 2
        btts = buscar_mercado_simples(driver, f"https://www.flashscore.com.br/jogo/{id_jogo}/odds/ambos-marcam/tempo-regulamentar/", "BTTS")
        
        resultado_dc = buscar_mercado_simples(driver, f"https://www.flashscore.com.br/jogo/{id_jogo}/odds/double-chance/tempo-regulamentar/", "DC")
        dc_1x, dc_x2 = resultado_dc if isinstance(resultado_dc, tuple) else ("N/A", "N/A")
