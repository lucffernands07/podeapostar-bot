...
async def executar_robo():
    ...
    ligas_config = {
        ...
        # --- NACIONAIS DEMAIS (Peso 2) ---
        203: ("Süper Lig (Turquia)", 2), 172: ("Bulgária", 2), 265: ("Chile", 2), 
        239: ("Colômbia", 2), 233: ("Egito", 2), 141: ("LaLiga 2", 2), 72: ("Brasileirão B", 2),
        
        # --- MATA-MATA / COPAS (Peso 1) ---
        13: ("Libertadores", 1), 11: ("Sudamericana", 1), 
        144: ("Copa do Brasil", 1), 45: ("FA Cup (Ing)", 1), 48: ("League Cup (Ing)", 1),
        143: ("Copa del Rey (Esp)", 1), 137: ("Coppa Italia (Ita)", 1), 
        81: ("DFB Pokal (Ger)", 1), 66: ("Coupe de France (Fra)", 1)
    }
    ...
    for l_id, (l_nome, l_peso) in ligas_config.items():
        # Temporadas: 2026 para Américas, 2025 para Europa e Copas
            ...
            for m in matches:
                ...
                if p:
                    ...
                    if p['o25'] >= 75:
                        ...
                        mercado, prio_mercado = "🤝 Ambas Marcam — 4/5 (Est.)", 85
                    elif p['win_home'] >= 75 or p['win_away'] >= 75:
                        time = t1 if p['win_home'] >= 75 else t2
                        mercado, prio_mercado = f"🛡️ Dupla Chance — {time} ou Empate", 75
                    elif "corners" in (p['conselho'] or "").lower():
                        mercado, prio_mercado = "🚩 +8.5 Escanteios — Volume Alto", 70
                    
                    if mercado:
                        prio_final = prio_mercado + (l_peso * 1000) # Peso da liga manda mais agora
                        ligas_no_bilhete.add(l_nome)
                        jogos_analisados.append({
                            "prio": prio_final, "liga": l_nome,
